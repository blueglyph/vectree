...
mod borrows {
    ...
    /// for mut inode in tree.iter_post_depth_mut() { // | for mut inode in tree.iter_post_depth_mut() { // cannot borrow `tree` as mutable because it is also borrowed as immutable
    ...
    /// use vectree::VecTree;
    ///
    /// let mut tree = VecTree::<i32>::new();         // | let mut iter = tree.iter_post_depth_simple();
    /// let mut iter = tree.iter_post_depth_simple(); // |                ---- immutable borrow occurs here
    ...
    /// use vectree::VecTree;
    ///
    /// let mut tree = VecTree::<i32>::new();   // | let mut iter = tree.iter_post_depth();
    /// let mut iter = tree.iter_post_depth();  // |                ---- immutable borrow occurs here
    /// let a1_borrowed = iter.next().unwrap(); // | let a1_borrowed = iter.next().unwrap();
    /// tree.clear();                           // | tree.clear();
    /// let value = a1_borrowed.deref();        // | ^^^^^^^^^^^^ mutable borrow occurs here
    /// println!("value: {value}");             // | let value = a1_borrowed.deref();
    ...
    /// let mut iter = tree.iter_post_depth();  // |
    /// let _a1 = iter.next();                  // | let mut iter = tree.iter_post_depth();
    ...
    ///
    /// let mut tree = VecTree::<i32>::new();
    /// let mut iter = tree.iter_post_depth_mut();    // |
    /// let _a1 = iter.next().unwrap();               // | let mut iter = tree.iter_post_depth_mut();
    ...
    /// let a1_borrowed = tree.get(4);           // | let mut iter = tree.iter_post_depth_mut();
